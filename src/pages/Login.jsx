    ...
    <div className="auth-container">
      <div className="auth-card fade-in">
        ...
        <div className="auth-logo">
          ...
          <h1>University Talent Hub</h1>
          <p>Platform Ekosistem Talenta Mahasiswa</p>
        </div>
        ...
        <div style={{ display: 'flex', alignItems: 'center', gap: '1rem', marginBottom: '1.25rem' }}>
          ...
          <span style={{ fontSize: '0.78rem', color: 'var(--text-muted)', fontWeight: 500 }}>atau masuk dengan email</span>
          <div style={{ flex: 1, height: 1, background: 'var(--border)' }} />
        </div>
        ...
        <form onSubmit={handleSubmit} style={{ display: 'flex', flexDirection: 'column', gap: '1rem' }}>
          ...
          <button type="submit" className="btn btn-primary btn-lg w-full" disabled={loading} style={{ marginTop: '0.25rem' }}>
            {loading ? <><div className="spinner" /> Masuk...</> : 'Masuk'}
          </button>
        </form>

        <p style={{ textAlign: 'center', marginTop: '1.5rem', fontSize: '0.875rem', color: 'var(--text-muted)' }}>
          Belum punya akun?{' '}
          <Link to="/register" style={{ color: 'var(--primary)', fontWeight: 700 }}>Daftar sekarang</Link>
        </p>
      </div>
    </div>
  );
